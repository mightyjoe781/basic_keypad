...
local machines_TTL = basic_keypad.machines_TTL or 16
...
function minetest.is_protected(pos, digger)
	...
	if is_protected then -- only if protected
		...
		if minetest.get_node(p).name == "basic_keypad:distributor" then -- attempt to activate distributor at special grid location: coordinates of the form 10+20*i
			...
			if meta:get_int("active1") == 0 then -- first output is disabled, indicating ready to be used as event handler
				if meta:get_int("x1") ~= 0 then -- trigger protection event
					...
					local table = minetest.registered_nodes["basic_keypad:distributor"];
...
minetest.register_on_chat_message(function(name, message)
	...
	if minetest.get_node(p).name == "basic_keypad:distributor" then -- attempt to activate distributor at special grid location: coordinates of the form 20*i
			local meta = minetest.get_meta(p);
			if meta:get_int("active1") == 0 then -- first output is disabled, indicating ready to be used as event handler
